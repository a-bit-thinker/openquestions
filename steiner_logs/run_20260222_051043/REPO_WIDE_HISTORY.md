...
Generated (UTC): 2026-02-22T05:10:43.570350+00:00
Log root: steiner_logs
Current run dir: /root/openquestions/steiner_logs/run_20260222_051043

- Total run dirs found: 20
- Prior run dirs found: 19
...
## Current Run Round1 Research Delta (`run_20260222_051043`)
- Mode: research-only (proof-first cache build, no certificate attempt).
- Instance: `S(6,8,29)` with admissibility pass (`expected_blocks=16965`, `lambda_5=8`).

### Reused from prior runs (explicit)
- Hard divisibility gate before any heavy search.
- Bounded symmetry diagnostics before randomized construction.
- Strict feasibility invariants as baseline quality constraints.

### New in this run (explicit)
- Five-seed proof-first stack specialized for `S(6,8,29)`.
- Three-loop critical-gap verification protocol with gate-specific exit rules.
- Finite-size bridge obligation promoted to explicit lemma in the proof outline.

### Practice blocker mapping added this run
1. Binary-only symmetry stalls on non-binary orbits.
- Source: Kramer-Mesner weighted orbit literature.
- Implementation delta: weighted-orbit micro-ILP neighborhoods after binary probe failure.

2. Add-only plateau at saturated `(r-1)` motifs.
- Source: iterative absorption framework.
- Implementation delta: pressure-triggered early handoff to destroy/repack with reserved slack.

3. Residual exact-cover attempted while residual is too large.
- Source: absorption-based staged closure logic.
- Implementation delta: explicit residual-eligibility gate before exact completion.

### Transfer value
- Rounds2+ now have a fixed stage order, selector rubric, and required metrics:
- point degree spread,
- `(r-1)` pressure tails,
- uncovered/overcovered movement.
